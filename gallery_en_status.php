...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["status"])) {
    ...
    echo "<script>alert('Changes saved successfully');window.location.href='gallery_en_fetch.php';</script>";

} else {
    // If no checkboxes were submitted (all entries unchecked), update all statuses to 0 (disabled)
    $sql_update_all_disabled = "UPDATE gallery SET status='0'";
    $conn->query($sql_update_all_disabled);

    echo "<script>alert('All entries disabled. Changes saved successfully');window.location.href='gallery_en_fetch.php';</script>";
