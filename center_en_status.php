...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["status"])) {
    ...
    $sql = "SELECT t_Center_id FROM centers";
    $result = $conn->query($sql);
    $existing_ids = [];

    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $existing_ids[] = $row["t_Center_id"];
        }
    }
    ...
    foreach ($existing_ids as $id) {
        ...
        $sql_update = "UPDATE centers SET status='$checked' WHERE t_Center_id='$id'";
        $conn->query($sql_update);
    }

    echo "<script>alert('Changes saved successfully');window.location.href='center_en_fetch.php';</script>";
} else {
