...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["status"])) {
    ...
    $sql = "SELECT t_reg_id FROM registration";
    $result = $conn->query($sql);
    $existing_ids = [];

    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $existing_ids[] = $row["t_reg_id"];
        }
    }
    ...
    foreach ($existing_ids as $id) {
        ...
        $sql_update = "UPDATE registration SET status='$checked' WHERE t_reg_id='$id'";
    ...
    $sql_update_all_disabled = "UPDATE registration SET status='0'";
    $conn->query($sql_update_all_disabled);

    echo "<script>alert('Changes saved successfully');window.location.href='stud_fetch.php';</script>";
